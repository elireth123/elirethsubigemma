...
function AIO_SendAddonMessage(msg, player)
    if AIO_SERVER then
        -- server -> client
...
function AIO_SendGenMessage(prefix, msg, MyType, player)
    if AIO_SERVER then
        return
    else
        -- print ("[DEBUG]465:AIO_SendGenMessage:[" .. msg .. "][" .. MyType .. "]")
		if player == nil then
           SendAddonMessage(prefix, msg, MyType)
        else		
           SendAddonMessage(prefix, msg, MyType, player )
		end   
    end
end
...
if AIO_SERVER then
    function SendAddonMsgGroup ( prefix, msg, Type, player )
	   ...
	   for LoopVar=1,NumGrPj do
	      player:SendAddonMessage ( prefix, msg, Type, GrPj[LoopVar] )
          PrintInfo("[DEBUG]SendAddonMsgGroup:850[" .. tostring(GrPj[LoopVar]) .. "]Sending")
	   end   
    end
    ...
    function frame:OnEvent(event, addon)
        if event == "ADDON_LOADED" and addon == "AIO_Client" then
            ...
            if tocversion and tocversion >= 40100 and RegisterAddonMessagePrefix then
				...
				RegisterAddonMessagePrefix("PRTXT")
            end
