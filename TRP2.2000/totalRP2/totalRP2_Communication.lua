...
function TRP2_SecureSendAddonMessage(Prefixe,Message,Cible,prior)
    -- print( "TRP2_SecureSendAddonMessage:[" .. Message .. "]" )
	...
	SendAddonMessage(TRP2_COMM_PREFIX,Prefixe..TRP2_ReservedChar..Message, "WHISPER", Cible);
end
...
function TRP2_SecureSendAddonMessageGroup(Prefixe,Message, prior)
	...
	if GetNumRaidMembers() > 0 then
		SendAddonMessage(TRP2_COMM_PREFIX,Prefixe..TRP2_ReservedChar..Message, "RAID");
	elseif GetNumPartyMembers() > 0 then
		SendAddonMessage(TRP2_COMM_PREFIX,Prefixe..TRP2_ReservedChar..Message, "PARTY");
	end
end
...
function TRP2_receiveMessage(message,sender)
    ...
    -- print ("TRP2_receiveMessage : [".. tostring(message).."] "..sender);
	local prefixe = string.sub(tostring(message), 1, string.find(message,TRP2_ReservedChar)-1);
	message = string.sub(tostring(message),string.len(prefixe)+2);
